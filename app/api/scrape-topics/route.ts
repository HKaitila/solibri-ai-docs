...
async function scrapeHelpCenter(): Promise<ScrapedTopic[]> {
  ...
  let topicCounter = 0; // Global counter for unique IDs
  ...
  for (const category of categories) {
    try {
      ...
      for (const match of matches) {
        ...
        // Create unique key: combine article ID with slug to ensure uniqueness
        // If still duplicates, append counter
        let uniqueKey = `${articleId}-${slug}`;
        let keyCounter = 0;
        
        while (seenIds.has(uniqueKey)) {
          keyCounter++;
          uniqueKey = `${articleId}-${slug}-${keyCounter}`;
        }
        ...
        const topic: ScrapedTopic = {
          id: uniqueKey, // Unique key combining article ID and slug
          title: slug
            .replace(/-/g, ' ')
            .split(' ')
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (ids.length !== uniqueIds.size) {
      console.warn(`WARNING: Found ${ids.length - uniqueIds.size} duplicate IDs after scraping`);
    }
    ...
    return NextResponse.json(
      {
        ...
        metadata: {
          ...
          has_duplicates: ids.length !== uniqueIds.size,
        }
      },
      {
        headers: {
